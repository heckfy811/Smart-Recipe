<template>
    <section class="recipes">
      ...
      <!-- Сообщение, если нет опробованных рецептов -->
      <p v-else class="empty-message">Вы ещё не опробовали ни одного рецепта.</p>
    </section>
  </template>
  ...
  export default {
    ...
    data() {
      return {
        recipes: recipesData,
        triedIds: JSON.parse(localStorage.getItem("triedRecipes")) || [],
      };
    },
    computed: {
      // Получаем опробованные рецепты
      triedRecipes() {
        return this.recipes.filter((recipe) =>
          this.triedIds.includes(recipe.id)
        );
      },
      // Ограничиваем отображение до 3 рецептов
      displayedRecipes() {
        return this.triedRecipes.slice(0, 3);
      },
    },
    methods: {
      getStars(rating) {
      ...
      toggleFavourite(recipeId) {
        if (this.triedIds.includes(recipeId)) {
          this.triedIds = this.triedIds.filter((id) => id !== recipeId);
        } else {
          this.triedIds.push(recipeId);
        }
        localStorage.setItem("triedRecipes", JSON.stringify(this.triedIds));
      },
      isFavourite(recipeId) {
        return this.triedIds.includes(recipeId);
      },
